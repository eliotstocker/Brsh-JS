...
class Terminal {
    constructor(options) {
        ...
        this._working = false;
        this._interactiveCommand = false;
    ...
    _statusCheck(status) {
        ...
        } else if (status === Shell.STATUS_INTERACTIVE) {
            this._clearCursor();
            this._working = true;
            this._interactiveCommand = true;
            this._newPrompt(this._setCurrentLine.bind(this, this._buffer));
        } else {
            this._clearCursor();
            this._working = true;
    ...
    _keyEvent(onUpdate, key) {
        ...
        if (key.length === 1) {
            ...
            if ((!this._working && !this._animating) || this._interactiveCommand) {
                onUpdate(this._buffer);
            }
        } else if (key === 'Enter' && (!this._working || this._interactiveCommand)) {
