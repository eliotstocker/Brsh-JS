...
class Terminal {
    ...
    _animateType(text, type) {
        ...
        const content = this._ansi.ansi_to_html(text);
    ...
    _clearCursor() {
        if(this._cursor !== 'none') {
            ...
            cursor && cursor.parentElement.removeChild(cursor);
        }
    }
    ...
    _listenForKeyEvents(onUpdate, el) {
        ...
        el.addEventListener('keyup', (e) => {
            if (e.key.length === 1) {
            ...
            } else if (e.key === 'Tab' && !this._working && !this._animating) {
                ...
                if(autoComplete) {
                    ...
                    if(options.length === 1) {
                        commandParts.push(`${path}/${options[0]}`);
