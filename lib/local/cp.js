...
class Cp extends LocalCommand {
    ...
    run() {
        ...
        let rSeen = false;
        let hSeen = false;
        let childSymlinkModeOverride = null;  // tracks last -L/-P/-d for child mode when -H is last
        const paths = [];
            ...
            if (arg.startsWith('-') && arg.length > 1) {
                for (const ch of arg.slice(1)) {
                    switch (ch) {
                        ...
                        case 'a': recursive = true; rSeen = true; symlinkMode = 'preserve'; childSymlinkModeOverride = 'preserve'; break;
                        case 'd': case 'P': symlinkMode = 'preserve'; childSymlinkModeOverride = 'preserve'; break;
                        case 'L': symlinkMode = 'follow'; childSymlinkModeOverride = 'follow'; break;
                        case 'H': symlinkMode = 'follow_cl'; hSeen = true; break;
                        case 'f': break;
                    }
                }
            } else {
        ...
        // -H always takes effect for CL-level symlinks (follow), regardless of which mode wins.
        // Child mode comes from the overriding -L/-P flag, or 'preserve' if H alone.
        // When H is not present, both levels use resolvedMode.
        const clItemMode = hSeen ? 'follow' : (resolvedMode === 'follow_cl' ? 'follow' : resolvedMode);
        const childMode = (hSeen || resolvedMode === 'follow_cl')
            ? (childSymlinkModeOverride !== null ? childSymlinkModeOverride : 'preserve')
            : resolvedMode;
