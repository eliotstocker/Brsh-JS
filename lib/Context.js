...
class Context {
    constructor(options) {
        ...
        const {cwd = '/', clearFn, destroyFn, onFsChange} = this.options;

        this.vars = {};
        this.aliases = {};
        this.fs = new FileSystem(cwd, {}, {}, onFsChange || null);
    ...
    setNodeFilesystem(cwd) {
        const NodeFileSystem = require('./NodeFileSystem');
        const onChange = this.fs._onChange;
        this.fs = new NodeFileSystem(cwd, onChange);
    }

    setFilesystem(filesystem, permissions = {}) {
        const commands = this.fs.getFileByPath(this.getVar('PATH')) || {};
        const oldPermissions = this.fs.getRawPermissions();
        const onChange = this.fs._onChange;
        this.fs = new FileSystem(this.fs.cwd, filesystem, Object.assign({}, oldPermissions, permissions), onChange);
