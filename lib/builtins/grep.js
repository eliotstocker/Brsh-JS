...
class Grep extends Command {
    ...
    run() {
                ...
                for (let j = 0; j < flags.length && !skipRest; j++) {
                    switch (flags[j]) {
                        ...
                        case 'e': {
                            ...
                            const eVal = rest.length > 0 ? rest : args[++i];
                            for (const p of eVal.split('\n')) { if (p.length > 0) patterns.push(p); }
                            patternSourceGiven = true;
                            skipRest = true;
                            break;
                        }
                        case 'f': {
        ...
        if (patterns.length === 0 && !patternSourceGiven) {
            if (paths.length === 0) {
                this.stdErr = 'grep: no pattern given';
                this.exitCode = 2;
                return Promise.resolve();
            }
            const posArg = paths.shift();
            for (const p of posArg.split('\n')) { if (p.length > 0) patterns.push(p); }
            if (patterns.length === 0) patterns.push(posArg); // preserve empty-string pattern
        }
        ...
        const collectFiles = (p, topLevel = false) => {
            ...
            if (entry.constructor === Object) {
                ...
                // During recursion, don't follow symlinks to directories
                if (!topLevel && this.fs.isSymlink && this.fs.isSymlink(p)) return;
                for (const name of Object.keys(entry)) {
                    collectFiles(p.replace(/\/$/, '') + '/' + name, false);
                }
                return;
            }
        ...
        const multiSource = recursive || sources.length + (hasError ? 1 : 0) > 1;
        let anyMatch = false;
