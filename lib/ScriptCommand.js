...
class ScriptCommand extends LocalCommand {
    ...
    run() {
        ...
        this.sandbox.context.setVar("#", this.arguments.length);
        this.sandbox.context.setVar("@", this.arguments.join(" "));
        ...
        return lines.reduce((chain, line) => {
            ...
            .then(() => {
                if(this.source) {
                    this.sandbox.context.unsetVar("#");
                    this.sandbox.context.unsetVar("@");
                    this.arguments.forEach((arg, index) => {
                        this.sandbox.context.unsetVar(`${index + 1}`);
                    });
